...
trait Permissions
{
    public function permissions_check(WP_REST_Request $request): WP_Error|bool
    {
        ...
        if (empty($token)) {
            return new WP_Error(
                'restfox_admin_empty_token',
                'restfox token is empty',
                ['status' => 401]
            );
        }
        ...
        if (!$user_id) {
            return new WP_Error(
                'restfox_admin_invalid_token',
                'restfox token is invalid or expired',
                ['status' => 401]
            );
        }
        ...
        if (!user_can($user, 'manage_options')) {
            return new WP_Error(
                'restfox_admin_user_not_allowed',
